...
router.post("/login", isNotLoggedIn, async (req, res) => {
  ...
  if (req.session.authenticated) {
    ...
    if (error) {
      ...
      if (
        (await Users.findOne({ userName: req.body.userName.toLowerCase() })) ==
        null
      ) {
        res.status(401).send("username  or password is in correct");
      } else {
...
router.delete("/logout", (req, res) => {
  if (req.session.authenticated) {
    (req.session.authenticated = false), (req.session.user = {});
    res.send("logged out");
  } else {
